...
--
-- Host: 127.0.0.1
-- Generation Time: 11-Ago-2017 às 03:39
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `confirmaTicket` (IN `email` VARCHAR(200), IN `subject` VARCHAR(200), OUT `temp` INT)  NO SQL
BEGIN 
declare temp1 bool;
declare temp2 bool;

select VerificaEmail(email) as temp1,VerificaSubject(subject) as temp2 from emails;

if (temp1 && temp2=1) THEN
 		SET temp = 2;
 else
 set temp=3;
 End IF;       
...
IF (idREs = 1)  THEN
 		update emails set emails.state = 2 where id= IdTicket;
 INSERT historicoestados(historicoestados.HoraAtribuicaoEstado,IdTicketEstado, IDEstadoEstado,IDFuncEstado,IdResTicket)
VALUES (now(), IdTicket, 5, emails.retornaIdMail(IDFuncEstado),idREs);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InserirTickets2` (IN `_mail` VARCHAR(100), IN `_from` VARCHAR(150), IN `_subject` VARCHAR(250), IN `_message` VARCHAR(700), IN `_user` VARCHAR(100), OUT `outa` INT(33))  Begin
  INSERT INTO emails
         (
              email,
           fromaddress,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LimparFicheiros` ()  NO SQL
Begin
truncate table upload;
End$$
...
CREATE DEFINER=`root`@`localhost` FUNCTION `RetornaId` (`_email` VARCHAR(300)) RETURNS INT(11) NO SQL
Begin
declare temp int;

  select id from emails where email like _email
  into temp;
return temp;
End$$
...
CREATE DEFINER=`root`@`localhost` FUNCTION `VerificaEmail` (`_email` TEXT) RETURNS INT(11) NO SQL
Begin
declare temp bool;
SET temp = 0;

 
    SELECT EXISTS(select `email` from emails where email like _email)
    INTO temp ;
    
return temp;
end$$

CREATE DEFINER=`root`@`localhost` FUNCTION `VerificaSubject` (`_subject` TEXT) RETURNS INT(11) NO SQL
Begin
declare temp bool;
SET temp = 0;

 
    SELECT EXISTS(select `subject` from emails where subject like _subject)
    INTO temp ;
...
--
ALTER TABLE `funcionario`
  MODIFY `id_funcionario` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=62;
--
